...
/* This file does not exist on the Raspberry Pi Broadcom implementation. */
#if HAVE_BCM_HOST != 1
...
#define CVPI_EGL_DEFAULT_PIXEL_FORMAT EGL_PIXEL_FORMAT_ARGB_8888_BRCM
  ...
  */
  ...
  /** 
      Broadcom pixel formats for reading and writing image data. The
      default, cvpi_egl_ARGB_8888, and is the required pixel format for
      CVPI.
  */
#if HAVE_BCM_HOST == 1
  ...
  */
  ...
  enum cvpi_egl_renderable_bits {
    ...
    /* OpenGL does not appear to be supported in Raspberry Pi Broadcom */
  ...
  enum cvpi_egl_renderable_api {
    ...
    /* If the user does not want to change the current rendering API */
  ...
  /* TODO: egl surface attribute settings */

  /* egl_settings->pixmap_surface_attrib_list,
     egl_settings->pbuffer_surface_attrib_list, and
     egl_settings_p->window_surface_attrib_list get set to NULL */
  ...
  */
  ...
  struct cvpi_egl_instance_s {
    ...
    EGLint minor;
    EGLint attrib_list[67];	/* 33 attributes + EGL_NONE, set by
				   generated header */
  ...
  */
  ...
  /* For settings whose types cannot be enforced statically, the
     ...
     if successful, otherwise returning an integer <= 0 if unsuccessful.
  */
