...
/* Add two channels, CHANNEL1 and CHANNEL2, in the same image. BIAS is
   divided by 255 before begin passed to vgConvolve. The result is
...
/* Adds the red, green, and blue channels. The result is copied to the
   ...
   before begin passed to vgConvolve. Each channel is scaled
...
/* Adds the red, green, blue, and alpha channels. The result is copied
   ...
   and 255. Each channel is scaled independently before adding. Bias
   only gets applied to ouput_channels. The values of unspecified
...
/* Same as cvpi_image_threshold_sector, but only applied to one channel. */
...
VGImage cvpi_image_mask_channel(const VGImage image, VGImageChannel maskChannel, CVPI_BOOL add);
  ...
  /* Computes a mean-like statistic by averaging vertically adjacent
     ...
     is done horizontally. ITTERATIONS is number of itterations of
     vertial averaging that should be done before returning the
     resulting image. If ITTERATIONS is negative, then horizontal
     averaging will be done. If ITTERATIONS is zero, the the resulting
...
VGImage cvpi_image_thicken(const VGImage image, VGubyte t_c, VGubyte f_c, CVPI_BOOL nonzero_true);
  ...
  /* Returns a pointer to data on the heap that is of the specified
     ...
     width* the IMAGE height * 8. If an error occurs, then NULL is
     returned. */
...
VGubyte* cvpi_image_cumulative_distribution(const unsigned int* histogram, VGint width, VGint height);
  ...
  /* Return a histogram equalized image. If SCALE is true, then values
